...
* [times_table_f] - Print a table with performance analysis results formatted in
plain text or in LaTeX (the latter requires the [siunitx], [multirow] and
[booktabs] packages).
...
with a function which parses the produced output. Otherwise, simply download the
benchmark [data][sort_data] produced the sorting algorithms and skip the actual
benchmarking steps within the examples.
...
plots is possible by using the data provided by [perfstats]. The following
sequence of instructions customizes the figure in the previous example:
...
% Add space for legend and put legend
ylim([1e-2 3e3]);
...
The examples in this section use the dataset available at 
...
The dataset corresponds to the results presented in reference [\[1\]](#ref1).
The following examples show how to replicate some of these results.
